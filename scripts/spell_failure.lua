...
function arcaneSpellFailure(nodeSpell)
    ...
    if ActorManager.isPC(rActor) then
        local nSomaticSpellFailureChance = DB.getValue(
                                               nodeSpellset.getChild('...'),
                                               'encumbrance.spellfailure') or 0

        local nSpellFailureEffects = EffectManager35E.getEffectsBonus(rActor,
                                                                      'SF', true) or
                                         0
        nSomaticSpellFailureChance = nSomaticSpellFailureChance +
                                         nSpellFailureEffects

        local nVerbalSpellFailureChance = 0
        if EffectManager35E.hasEffectCondition(rActor, 'Deafened') then
            nVerbalSpellFailureChance = 20
        end
        nVerbalSpellFailureChance = nVerbalSpellFailureChance +
                                        nSpellFailureEffects
        ...
        if nSomaticSpellFailureChance > 0 then
            ...
            local bArcaneCaster = isArcaneCaster(nodeActor, nodeSpellset) or
                                      EffectManager35E.hasEffectCondition(
                                          rActor, 'FSF')
    ...
    local sName = DB.getValue(nodeActor, 'name',
                              Interface.getString('spellfail_char_noname'))
    if bNoVerbal and bVerbalSpell then
        ChatManager.SystemMessage(string.format(
                                      Interface.getString(
                                          "spellfail_verbalwhensilenced"), sName))
    end
    if sCondition == 'pinned' and bSomaticSpell then
        ChatManager.SystemMessage(string.format(
                                      Interface.getString(
                                          "spellfail_somaticwhilepinned"), sName))
    elseif sCondition == 'pinned' then
        ChatManager.SystemMessage(string.format(
                                      Interface.getString(
                                          "spellfail_concentrationcheck"),
                                      sName, sCondition))
    end
...
local function spellFailureMessage(rSource, _, rRoll)
    ...
    if rRoll.nTarget then
        ...
        rMessage.text = rMessage.text ..
                            string.format(
                                Interface.getString("spellfail_failurethreshold"),
                                nTargetDC)
